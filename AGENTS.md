...
- `workspace <command> --json`—for example, `workspace context --json`—is the stable installed-product surface for any shell-capable harness: a content-free read lane plus a per-launch-authenticated act lane (`chat`, `chats list`, `manage`, `spaces create/register`, `files add`) that needs the running app. The `manage` group is the management conversation above all Spaces: machine-local transcript, personal capabilities plus app-materialized management instructions, same turn machinery, full-trust (taught to prefer the CLI, not tool-restricted). `npm run workspace:drive` is the separate real-Pi-turn test driver; do not conflate it with the installed management CLI.
