...
- Native Pi session parity inside the desktop: steer a running turn from the composer (Enter mid-turn; ⌘/Ctrl+Enter queues for after the turn instead), choose the Chat's thinking level from the composer or `/thinking`, run turns without a wall-clock cap, and have the Assistant's shell tools see your real login-shell `PATH` even when the app was launched from the Dock.
