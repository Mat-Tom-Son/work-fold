...
The bottom-rail **Add** menu has three entries — **Your Library**, **Skills & Extensions**, and **Apps** — so occasional destinations never crowd the primary rail. Skills & Extensions is the Assistant tools tab: discovery and management in one place, with installed tools grouped by where they live (Everywhere, meaning your fold and every Space, above This Space only) and a review step that shows that hierarchy before anything installs. Every Space may own one Library tab, and all of those tabs read the same passive personal collection. The owning Space is the default copy target, and a destination selector can send an independent copy to any registered Space. Provider credentials, model defaults, and Space instructions live under **Settings → Assistant**: connections are shared on the computer, while each Space and the fold keep separate model choices and each Space may add machine-local instructions for subsequent turns. Clicking a composer's model name opens that page already focused on the correct Space or fold selector. The fold can inspect and edit a Space's connected-model default and instructions through narrow receipted commands, but provider secrets and connection setup remain human-only. OpenRouter can refresh its tool-capable text-model list from the live provider catalog without storing the key in that cache. Connections used by a restricted Space app are configured separately with that app in the Space's **Apps** tab.
...
work-fold reserves two hidden support directories inside a Space: `.work-fold/` for the portable `space.json` identity and append-only conversation logs, and `.pi/` for native Pi project configuration. Neither appears in the Files surface or History checkpoints. History also leaves out version-control internals, installed dependencies, virtual environments, self-declared caches, folders you ignored in Files, and folders your `.gitignore` excludes, while still capturing every individual file. Removing a linked Space from the app leaves `.work-fold/` with the folder; deleting a managed Space deletes its folder normally.
...
- A menu-bar popover that opens your fold ("Your fold" in the menu bar and tray): its conversation stays visible and streams the Assistant's reply as it arrives, with direct **Open app** and **New chat** header actions. One expanding composer keeps the fold's actual model name and supported reasoning level beside its aligned action; clicking the model opens the fold-scoped selector in the main app. While work runs, that action becomes **Stop** and the text area remains available for drafting a follow-up. Drop files, folders, or links on the macOS menu-bar icon or anywhere over the popover — the whole surface becomes a drop target only during the drag — then add an instruction; with material staged, the send button reads "Fold it in". Follow the request through working, needs-you, handed-off, and done, with an explicitly attributed action trail backed by act receipts and a Stop that names everything it aborts. New chat starts a clean saved transcript without deleting the previous one. The Windows tray offers the same popover from its menu.
- Authority you choose: runnable-code, standing-power, and irreversible-destruction acts always enter a pinned, journaled decision path. **Reviewed** mode leaves them on a host-composed Needs-you card or exercises a narrow standing policy; **Unrestricted** mode executes every newly admitted act immediately, including permanent deletion. Approved browsers inherit the desktop's mode, every automatic decision is receipted with surface `unrestricted`, and only local Settings can change root authority.
- The glance: a deterministic digest of running work, needs-you items, what changed since you last looked, and Check status — composed by app code from recorded state only, with no model call, no file scanning, and no background watching, shown on the remote client's Needs you screen and in a compact main-window panel.
- Routings: declared cross-Space glue — a reviewed manual, interval, settle, or one-time trigger and up to eight deterministic steps (start a Space chat with a fixed message, copy files with a restore point, run a Check), executed by app code on the shared scheduler discipline with per-hop receipts. Settings → The fold → Routings shows state, schedule, steps, valid actions, and bounded run history. Enabling one uses the staged authority path; cross-Space work never runs inside a Space chat, because Space transcripts travel with their folders.
- Private-alpha web access to your fold ("Your fold on the web") at a chosen `<name>.work-fold.com`: set the address and password in Settings, approve each new browser with a matching code on the desktop, then open saved management Chats, browse filtered Space-relative file trees, and ask the one management Assistant to work across Spaces or delegate to their Assistants. The Space selection there never changes who the person is chatting with. Bounded encrypted uploads use short-lived app-owned staging until the management Assistant uses or places them through the normal restore-pointed path. New-address enrollment is controlled by the hosted bridge instead of an invitation code or a credential embedded in the app. Payloads cross the hosted service as application-encrypted signed envelopes rather than durable plaintext records, but the hosted client and bridge are trusted parts of this full-authority feature and an active service compromise is outside that protection. While a management turn runs, an up-to-date desktop streams bounded live response text and its current activity line to the approved browser inside those same envelopes, then reconciles the durable reply when the turn settles. The desktop must be online and remains the local execution endpoint.
- Share a page ("pages your fold serves"): after the staged authority path completes, one explicitly designated file is served as a rendered page at your `<name>.work-fold.com` address — live from your desktop through the relay, encrypted with a key carried in the link fragment, with an honest "asleep" state when the desktop is offline. Revoking kills every copy of the link; nothing else in the Space is exposed; there is no uptime promise, discovery, or App Store.
- Native OS file drops on any Chat composer: dropped files upload into that Space's dated `Dropped/` folder and attach as explicit chat context. Attached PNG, JPEG, GIF, and WebP files reach the model as images, and a pasted screenshot takes the same explicit upload-then-attach path.
- Native Pi session parity inside the desktop: steer a running turn from the composer (Enter mid-turn; ⌘/Ctrl+Enter queues for after the turn instead), choose a supported reasoning level from the Space or fold composer (or `/thinking` in a Space Chat), run turns without a wall-clock cap, and have the Assistant's shell tools see your real login-shell `PATH` even when the app was launched from the Dock.
...
`npm run desktop:make:mac` builds the non-interactive, separately identified `work-fold Local Smoke` Apple silicon structural candidate. `npm run desktop:rc:mac` builds a faster app-only Developer ID-signed/notarized candidate for interactive QA without creating distribution media. After CI passes for both the exact pushed `main` commit and its matching source tag, `npm run desktop:release:mac` builds, signs, notarizes, staples, verifies, and draft-first publishes the complete production artifacts to the separate public Mac feed; timed, source-bound checkpoints let an interrupted build resume without weakening final verification. A failed candidate tag is never moved or reused. The legacy Workspace updater proof is retained in the [macOS release runbook](docs/macos-release.md); work-fold requires its own signed first-install proof followed by a higher-version updater proof before the lane is accepted.
