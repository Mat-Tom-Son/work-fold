...
function run(rawArguments) {
  ...
  try {
    ...
    if (isActCommand(argumentsList)) {
      ...
      if (!actToken) {
        writeHandle($.NSFileHandle.fileHandleWithStandardError, `workspace: ${ACT_UNAVAILABLE_MESSAGE}\n`);
        $.exit(6);
      }
...
function performRequest(context, argv, actToken, payload) {
  ...
  try {
    ...
    while (!fileManager.fileExistsAtPath($(responsePath))) {
      if (Date.now() >= deadline) {
        throw transportTimeout(`Workspace did not answer CLI request ${requestId} within ${context.timeoutMs} ms.`);
      }
...
function readText(path) {
  ...
  if (!value) throw new Error(`Workspace CLI file could not be read at ${path}.`);
