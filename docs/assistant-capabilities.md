...
Current product copy calls a Folder's AI helper a **Worker**, the management
helper the **work-fold agent**, and the model/provider settings destination
**Agents**. This contract retains Assistant and `space` terminology where it
names existing Pi concepts, APIs, storage, commands, or schemas.
...
The CLI projection is intentionally compact and content-free. Capability writes continue through Pi's package/import operations and work-fold's trust and concurrency policies. See [work-fold management layer](management-layer.md) for the snapshot and protocol boundary.
...
|---|---|---|
| **Ordinary file** | A template, reference document, example, image, or reusable file in a Space | Passive content. It reaches a Chat only when a person attaches it or a tool reads it. |
...
Internal APIs and storage may retain the name `resource` for compatibility. User-facing copy uses the specific Pi term—**Skill**, **Extension**, prompt, or package—when it means Assistant configuration. The desktop no longer has a Library of reusable files (2026-09-25); the CLI act-lane `library` family and its server routes remain unchanged for now as a compatibility contract, pending a deliberate removal.
...
In product language, lead with the outcome (“install this Skill” or “add this Extension”) and show package source, scope, resource types, load state, and diagnostics as provenance. A package can be mixed: installing an item presented as a Skill may also load Extensions, prompts, themes, dependencies, or install scripts. The Discover details dialog must disclose that package boundary before installation: it shows the description, the where-chooser, what is inside (the Skills and Extensions found), one plain warning when the install can run code on the computer, and a collapsed **Technical details** section holding source, version and license, provenance, dependencies, and install scripts. Do not add **Packages** as a peer to Space or Skills & Extensions unless the product model is deliberately revisited.
