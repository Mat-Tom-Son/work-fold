...
remains outstanding, including after restart. Stop and expiry fence both
answer acceptance and continuation admission through every ancestor.
...
receive one bounded synthesis turn. Children finishing before their parent
...
## Limits (Settings → General → Limits)

The counts and transport sizes are fixed in this build and visible here. The
continuation switch and trash retention are configurable. An injected provider
budget is supported by the request store; there is no provider-budget setting
in the shipped UI.
...
- The fold has no independent ambient scheduler. A declared routing fold step may start a management request; its continuations obey the same limits.
