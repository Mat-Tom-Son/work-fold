...
- Show the configured provider/model before a Chat's first send, safely retry transient provider stream failures from completed tool results, preserve terminal partial output and activity as a resumable interruption, and save sanitized setup or unexpected failures in the transcript.
...
- In the private alpha, optionally reach saved management and Space Chats at a chosen private `<name>.work-fold.com` address while the desktop is online, after password sign-in and explicit one-time full-trust desktop approval for that browser; use bounded transcripts, filtered relative file trees, explicit context, and bounded uploads, with content-bearing operations carried in signed application-encrypted envelopes through a trusted hosted client and bridge.
