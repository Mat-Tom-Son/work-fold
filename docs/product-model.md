...
- Talk to the management conversation above all Spaces through `workspace manage send|status|result|wait|abort|list`: the same Assistant runtime with personal capabilities only, a machine-local transcript, default single-conversation behavior, and task-scoped outcomes.
