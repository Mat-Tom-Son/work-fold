...
**Remote access** is a private-alpha second surface over that same canonical management conversation, not a second Assistant, transcript, or cloud-synchronized Space. A person chooses one private `<name>.work-fold.com` address and password in Settings; alpha enrollment authority is supplied by the configured build rather than entered as an invitation code. Password sign-in establishes only a short-lived browser session; the desktop must separately approve that browser by matching a six-digit code, after which a non-exportable browser key and revocable desktop-signed grant authorize the surface. This is a full-trust grant because the management Assistant can use ordinary local tools. The browser can read the bounded recent management transcript, send or stop its exact request, list registered Space names, and inspect bounded ignored-file-aware Space-relative trees; truncation and filtering are disclosed in the browser. It cannot attach arbitrary paths, tunnel the local HTTP API, receive absolute roots, manage capabilities/settings, or call an open-ended method. Browser requests enter a dedicated closed semantic adapter and then the same turn-acceptance path as the menu-bar surface. The bridge persists account/session/grant/operation metadata but never durable message or file content; content projections cross it as signed application-encrypted envelopes. That protects persisted relay state and passive payload handling, not an actively compromised hosted client or bridge: the hosted origin serves the code that may use an approved browser key and is therefore inside this alpha's authority boundary. Revocation withdraws the desktop-local grant before the server mutation, is serialized with dispatch, and stops locally tracked work started by that grant. Revoking one browser, revoking every generation, disabling remote access, or deleting the address are separate desktop actions. Core Space use stays local and account-free.
...
- In the private alpha, optionally reach that same management conversation at a chosen private `<name>.work-fold.com` address while the desktop is online, after password sign-in and explicit one-time full-trust desktop approval for that browser; browse only bounded recent transcript data, names, and filtered relative file-tree metadata, with content-bearing operations carried in signed application-encrypted envelopes through a trusted hosted client and bridge.
