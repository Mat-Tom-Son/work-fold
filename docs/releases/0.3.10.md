...
work-fold 0.3.10 makes model controls truthful before the first message and
...
- Interrupted turns do not request a title. Manual and previously generated
  titles prevent another title request.
- If the model request fails, work-fold saves a bounded first-message fallback
  so it does not retry on later turns.
...
- Explicit integration coverage proves that an interrupted first turn does not
  request a title, the first successful turn requests exactly one, and later
  turns leave it unchanged.
