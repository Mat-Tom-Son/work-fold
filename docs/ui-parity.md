...
- Pending needs-you decisions render as host-composed cards in the popover, an anchored main-window flyout, and the approved remote client; deciding one is never available from the CLI, and a card exists only for a real staged act. The glance renders behind the popover footer's collapsed **What's new** strip — expanding the strip is what acknowledges it — plus the remote home and a compact main-window panel; the popover keeps its quiet door-first idle state, with decisions, the conversation, and the glance as one-at-a-time folded sections. None of these adds a rail destination, tab, in-window badge, or notification — the one exception is the optional menu-bar/tray attention dot for pending decisions, which ships only if the visual-acceptance pass admits it (see visual-design); with nothing pending, no control appears.
