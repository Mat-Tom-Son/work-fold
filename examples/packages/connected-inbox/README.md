...
Install this directory through the normal local-package flow in **Assistant tools** only when its source is trusted. Package scope determines whether it is Everywhere or belongs to This Space only; a project-scoped package loads while its folder is registered as a Space.
