...
Each helper call admits up to 8 PDF pages from a source up to 64 MiB. Omitted pages are explicit. Rendering is limited to 4 million pixels per page and scale at most 4. One tool result admits up to 4 PNGs, each at most 2 MiB, 8 MiB combined. Return text and logs each have a 64 KiB cap and explicit truncation. Scripts default to 60 seconds, with a maximum 120 seconds. Each worker has a 512 MiB JavaScript heap limit so an allocation loop fails within its run; native library and external buffer memory are outside this limit. Split longer documents into deliberate batches and release intermediate data. These limits are recovery boundaries, not a sandbox; ordinary library use remains available inside the full-trust script.
