...
async function createFilePresence(args: string[]): Promise<void> {
  ...
  const proposal = normalizeWorkFoldCheckProposal({
    ...
    check: {
      ...
      severity,
      trigger: "manual",
      sensor: { id: "work-fold.file-presence", revision: 1, parameters: { expect } },
...
async function validateProposal(args: string[]): Promise<void> {
  ...
  const proposal = normalizeWorkFoldCheckProposal(JSON.parse(await readFile(path, "utf8")));
  if (parsed.values.json) {
...
function printScope(proposal: WorkFoldCheckProposal): void {
...
function normalizeSeverity(value: string | undefined): WorkFoldCheckSeverity {
  if (value === "info" || value === "warning" || value === "error") return value;
  throw new Error("--severity must be info, warning, or error.");
}

function normalizeCreator(value: string | undefined): WorkFoldCheckProposal["createdBy"] {
...
function defaultProposalName(name: string): string {
  ...
  return `${slug}.work-fold-check.json`;
}

function printHelp(): void {
  process.stdout.write(`work-fold Check proposal tool
...
Create options:
  ...
  --out <path>           Defaults to <name>.work-fold-check.json
