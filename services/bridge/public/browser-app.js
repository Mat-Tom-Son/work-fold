...
export function createBrowserAppView({ read, online, resolve = async (app) => app }) {
  ...
  let healthTimer;
  let checking = false;
  const clear = () => { clearTimeout(loadTimer); clearInterval(healthTimer); generation++; frame = null; entry = null; content.replaceChildren(); inFlight = 0; checking = false; };
  ...
  function message(event) {
    ...
    if (data?.type === "work-fold.browser-app.ready" && entry !== null) {
      frame.contentWindow.postMessage({ type: "work-fold.browser-app.init", channel, html: entry }, "*");
      entry = null;
      return;
    }
    ...
    let valid = false;
    try { valid = kinds.includes(data.call?.kind) && JSON.stringify(data.call).length <= 1024; } catch {}
    if (!valid) return respond({ ok: false, code: "APP_DENIED", message: "This view can only read its reviewed app surface." });
    ...
    inFlight++;
    void call(data.call, current).then((result) => respond({ ok: true, result })).catch((error) => {
