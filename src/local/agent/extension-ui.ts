...
export function validateExtensionUiResponse(request: PiExtensionUiRequest, response: PiExtensionUiResponse): void {
  ...
  if (Buffer.byteLength(response.value, "utf8") > extensionUiLimits.answerBytes) throw new Error(`Extension answer exceeds the ${extensionUiLimits.answerBytes / 1024} KiB limit. See Settings → Automations → Limits.`);
...
export class RoutedPiExtensionUiBridge extends EventEmitter implements PiExtensionUiBridge {
  ...
  request(request: PiExtensionUiRequest): Promise<PiExtensionUiResponse> {
    ...
    if (this.pending.size >= extensionUiLimits.pendingTotal || [...this.pending.values()].filter((item) => scopeKey(item.request) === scopeKey(request)).length >= extensionUiLimits.pendingPerChat) {
      return Promise.reject(new Error(`Too many pending Extension questions (${extensionUiLimits.pendingPerChat} per Chat, ${extensionUiLimits.pendingTotal} total). Settings → Automations → Limits shows these bounds.`));
    }
    if (Buffer.byteLength(JSON.stringify(request), "utf8") > extensionUiLimits.requestBytes || (request.method === "select" && (!request.options.length || request.options.length > extensionUiLimits.options))) {
      return Promise.reject(new Error(`Extension question exceeds the ${extensionUiLimits.requestBytes / 1024} KiB or ${extensionUiLimits.options} choices limit. See Settings → Automations → Limits.`));
    }
  ...
  setEditorText(scope: PiExtensionUiScope, value: string): void {
    if (Buffer.byteLength(value, "utf8") > extensionUiLimits.answerBytes) throw new Error(`Extension editor exceeds the ${extensionUiLimits.answerBytes / 1024} KiB limit. See Settings → Automations → Limits.`);
