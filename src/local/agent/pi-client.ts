...
export function createRestrictedAppProposalTool(input: {
  ...
  return {
    ...
    promptGuidelines: [
      ...
      "The package must contain package.json with an agentApp path and already-built local assets; Workspace never runs npm or installs dependencies. agent-app.json version 2 has id, title, optional description, runtime {kind:'sandboxed-web',entry,worker?}, ui {icon?}, tools, automations, and permissions {network,files,notifications?}. Each automation has id, title, optional description, handler, trigger {kind:'interval',intervalMinutes:15..1440}, explicit network/file/notification permission-id subsets, catchUp:'none'|'latest', and overlap:'skip'. A notification is {id,title,description} with static single-line reviewed copy and must be referenced by an automation. A file permission is {id,target:'file'|'directory',access:'read'|'read-write'}. A network permission has id, target ({kind:'public-https',origin} or {kind:'loopback-http',host:'127.0.0.1'|'::1',port}), explicit GET/POST/PUT/PATCH/DELETE methods, and auth. Public auth supports none, api-key {header}, bearer, basic, or oauth2-pkce {issuer,clientId,scopes}; loopback is anonymous only. Never put a secret in the package.",
