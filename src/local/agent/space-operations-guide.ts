...
export function workFoldSpaceOperationsGuide(executable = "work-fold"): string {
  ...
  return [
    ...
    `- Pass \`--space <this Space's id>\` on every Space-scoped command. Your turn context names that id, and \`${cmd} context --json\` reports it too. Never rely on a working directory.`,
    ...
    `- \`${cmd} chat answer --space <id> --question <id> --answer "<text>" --json\` delivers your answer to a question that is waiting on you and starts that asker's one continuation. A second answer, an expired question, and a question that is not yours are refused.`,
    ...
    `- \`${cmd} chat wait --space <id> --task <id> [--timeout <seconds>] --json\` follows a task you started until it finishes or is left waiting, and tells you which. What comes back is that task's released result — never its Chat and never its folder.`,
    "",
