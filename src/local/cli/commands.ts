...
export function workFoldCliHelp(productName = "work-fold", topic?: string): string {
  ...
  if (normalizedTopic === "collaborate" || normalizedTopic === "collaboration") {
    return [
      ...
      "answer past the request's window, and an answer sent from another Space",
      ...
      "means a fresh Chat with copies.",
      "",
      ...
      "comes back (ids here are examples; use the ones each command returns):",
      "",
      ...
      `  $ ${executable} chat answer --space space-0000000000000002 --question question-0000000000000001 --answer "Use the November figures." --json`,
      `  $ ${executable} chat wait --space space-0000000000000002 --task task-child --json`,
      `  $ ${executable} chat report --space space-0000000000000002 --task task-child --summary "Drafted drafts/q3-note.md from the brief." --file drafts/q3-note.md --outcome succeeded --json`,
      `  $ ${executable} requests show --request request-0000000000000001 --json`,
      "",
      "Limits are generous defaults you can see in Settings → The fold →",
      "Limits, and every refusal names the one it hit: a request stays open",
      "for 24 hours, one request hands out 32 tasks at most, work can be",
      "handed on 4 deep, 8 run at once, and the fold is brought back at most",
      "4 times per request. Later finishes are still recorded.",
      "",
    ].join("\n");
  }
  if (normalizedTopic === "requests" || normalizedTopic?.startsWith("requests ")) {
    return [
      ...
      "above Spaces, so neither verb takes --space, and both are reads —",
      "nothing here starts or stops work. list shows recent roots, newest",
  ...
  if (normalizedTopic === "routings" || normalizedTopic?.startsWith("routings ")) {
    return [
      ...
      "Debounce: 2–120 seconds. Cooldown: 1–1440 minutes. Up to 16 steps and",
      "8 runs at once by default.",
      "",
    ].join("\n");
  }
  if (normalizedTopic === "pages" || normalizedTopic?.startsWith("pages ")) {
    return [
      ...
      "snapshot-off only reduce exposure and run immediately.",
      "Needs the work-fold app running.",
      "",
    ].join("\n");
  }
  if (normalizedTopic === "trash" || normalizedTopic?.startsWith("trash ")) {
    return [
      ...
      "Settings → The fold → Recently deleted). restore puts an item back where",
