...
export function workFoldCliHelp(productName = "work-fold", topic?: string): string {
  ...
  if (normalizedTopic === "spaces" || normalizedTopic?.startsWith("spaces ")) {
    return [
      ...
      "folder and records a receipt. appearance apply imports a typed",
  ...
  if (normalizedTopic === "files" || normalizedTopic?.startsWith("files ")) {
    return [
      ...
      "stay undoable. Needs the work-fold app running.",
      "",
    ].join("\n");
  }
  ...
  if (normalizedTopic === "routings" || normalizedTopic?.startsWith("routings ")) {
    return [
      ...
      "conversation. They sit above Spaces, so no --space. enable turns on",
      "the declaration at its exact reviewed content and records a receipt;",
      "run starts one bounded run now; stop, disable, and delete only narrow",
      "standing behavior and run immediately. receipts lists per-run",
      ...
      JSON.stringify({
        kind: "work-fold.routing-proposal", version: 3, name: "Brief handoff",
  ...
  if (normalizedTopic === "pages" || normalizedTopic?.startsWith("pages ")) {
    return [
      ...
      "desktop at your work-fold address. pages stage shares the page",
      "immediately and records a receipt; stage-app puts one installed App",
