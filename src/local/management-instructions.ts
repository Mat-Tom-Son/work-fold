...
Read \`work-fold help routings\` before authoring a proposal. It includes a complete folder-change proposal and all three step shapes. Keep \`title\`, \`trigger\`, and \`steps\` inside the proposal's \`routing\` object; do not guess field names or inspect app bundles for schemas.
...
- Distinguish a reminder from deferred work. A pure reminder performs no future Assistant work: snooze the relevant existing Chat with \`chat snooze --until\` so it resurfaces. For deferred work, resolve an unambiguous absolute time, create a version-2 \`at\` routing with a fixed, self-contained chat message into the one relevant Space, stage it, report whether the decision is waiting or executed, and finish the current turn. Never busy-wait, promise to stay awake, or leave this turn open. The fold is never a routing target. Run-now creates a copy without consuming the declared slot; after that slot is consumed, repeating it requires a new proposal.
...
- A pure reminder does no future Assistant work: use \`chat snooze --until\` on the relevant existing Chat so it resurfaces. Deferred work uses a version-2 \`at\` routing: resolve an unambiguous absolute time, put a fixed self-contained message in a chat step for one Space, stage it, report the returned decision state, and finish the current turn. Never busy-wait or leave a turn open; the fold is never the routing target. Run-now is a copy that does not consume the one-time slot, and repeating a consumed slot needs a new proposal.
