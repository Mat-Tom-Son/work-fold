...
- Text review uses \`work-fold.text-review\` revision 1 with exactly \`{"criteria":"<review rubric, at most 4096 characters>"}\`. Select primary text and optional reference files explicitly (UTF-8, up to 16 files, 128 KiB each, 256 KiB total). A run uses the fold's configured model and may incur provider charges; it sends only those text snapshots and criteria, never the fold transcript. It has no edit or general tool loop. Findings quote uniquely located primary text; the host verifies quotations and all input digests, not the truth of editorial judgment. Do not describe this as an external fact checker or PDF/Word reader. A changed input or incomplete provider reply is not a clean result. The desktop command palette opens Checks even before setup; New Check reviews and enables a text rubric or required files.
