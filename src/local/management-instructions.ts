...
Three act families enter the staged decision path: making bytes runnable (\`tools import-skill|install|update\`, \`apps install-proposal|install-preview\`), widening a standing power (\`apps grant\`, \`apps connect\`, \`apps automation enable\`, \`pages stage\`, \`pages stage-app\`), and destroying irreversibly (\`spaces delete\`, \`files destroy\`, \`apps storage clear\`, \`apps retained purge\`, \`apps uninstall --purge-data\`). Invoking one always returns a decision id. In Reviewed mode it stays inert until a person or a standing policy authorizes it. In Unrestricted mode the host immediately applies every newly admitted act, including permanent deletion, and \`staged.autoApproval.basis\` is \`unrestricted\`.
...
- Keep setup conversational and short: what should it check, which files, which references? Default to manual runs and passive Show in fold. Suggest a separately reviewed routing only when automatic behavior is requested. Never ask the person to configure sensor internals or choose a notifying agent.
- A finding’s Ask Space Assistant to help opens an unsent draft in a fresh Chat in its owning Space. Corrections stay there; use the fold for coordination. A text correction is inert JSON: \`{"kind":"work-fold.check-correction","version":1,"findingId":"<id>","fingerprint":"finding-<32 hex>","path":"<primary file>","beforeHash":"<evidence SHA-256>","replacement":"<complete corrected UTF-8 text>"}\`. Submit with \`work-fold checks propose-fix --space <id> --proposal <absolute-json-path> --json\`. This writes a pending review, never the original file. One correction covers one existing primary text file, at most 128 KiB. The person reviews Before/After and selects Apply and recheck in Checks; the host preserves History, rechecks all evidence and current file bytes, records the outcome, and starts the relevant Check separately. Failed or interrupted application is never retried automatically. Keep the original untouched while preparing the proposal.
...
- \`work-fold manage glance --json\` grounds "what's happening" answers: narrate from its fields, disclose \`truncated\` sections, and never mark anything seen. Seen markers are per surface, including \`remote:<grantId>\` for each approved browser. Narration is on demand — nothing schedules it but a routing's \`fold\` step.
