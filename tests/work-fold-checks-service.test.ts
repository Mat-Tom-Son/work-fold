...
test("optional Checks complete proposal, grant, task, evidence, decision, stale, and clear lifecycle", async (t) => {
  ...
  const space = { id: "space-delivery", spaceRoot: root };
  ...
  const rerun = await service.run({ space, checkId: enabled.declaration.id, actor: { kind: "cli", spaceId: space.id } });
  ...
  const recurrence = await service.run({ space, checkId: enabled.declaration.id, actor: { kind: "cli", spaceId: space.id } });
...
test("accepted and running Check work is pending rather than a content or infrastructure error", async (t) => {
  ...
  t.after(() => service.close());
  const space = { id: "space-running", spaceRoot: root };
  const enabled = await service.enable({ space, proposalPath, actor: "human" });
...
test("a rejected Check store creation is evicted so a repaired store can be retried", async (t) => {
  ...
  const service = new WorkFoldCheckService({
    ...
    storeFactory: async (spaceId) => {
      attempts += 1;
      if (attempts === 1) throw new Error("damaged state");
      return WorkFoldCheckStore.create(spaceId, { path: join(sandbox, "repaired.json") });
    },
  });
  t.after(() => service.close());
  const space = { id: "space-retry", spaceRoot: root };
...
test("changed portable declarations lose exact machine authority", async (t) => {
  ...
  t.after(() => service.close());
  const space = { id: "space-digest", spaceRoot: root };
  const enabled = await service.enable({ space, proposalPath, actor: "human" });
...
test("damaged declarations are health errors, never an unconfigured or clear state", async (t) => {
  ...
  const status = await service.status({ id: "space-damaged", spaceRoot: root });
...
test("the service binds a stable Space id to its registered folder", async (t) => {
  const sandbox = await mkdtemp(join(tmpdir(), "work-fold-check-space-binding-"));
  const root = join(sandbox, "Space");
  ...
  await assert.rejects(
    () => service.status({ id: "space-bound", spaceRoot: wrongRoot }),
  ...
  await assert.rejects(
    () => service.status({ id: "space-bound", spaceRoot: root }),
...
test("admission failure makes the run and status unhealthy, and status never executes a sensor", async (t) => {
  ...
  t.after(() => service.close());
  const space = { id: "space-health", spaceRoot: root };
  const enabled = await service.enable({ space, proposalPath, actor: "human" });
...
test("terminal persistence failure retains the task fence until task polling repairs it", async (t) => {
  ...
  const otherStore = await WorkFoldCheckStore.create("space-other", { path: join(sandbox, "other-state.json") });
  ...
  const service = new WorkFoldCheckService({
    kernel: new WorkFoldKernel(),
    storeFactory: async (spaceId) => spaceId === "space-recovery" ? store : otherStore,
...
test("run-all supports the full declaration authority ceiling", async (t) => {
  ...
  t.after(() => service.close());
  const space = { id: "space-many", spaceRoot: root };
  ...
  const accepted = await service.run({ space, actor: { kind: "renderer", spaceId: space.id } });
...
test("run admission is synchronously reserved for every adapter", async (t) => {
  ...
  t.after(() => service.close());
  const space = { id: "space-reservation", spaceRoot: root };
  const enabled = await service.enable({ space, proposalPath, actor: "human" });
...
test("recorded findings cannot cross into a folder that later becomes another Space", async (t) => {
  const sandbox = await mkdtemp(join(tmpdir(), "work-fold-check-late-nested-space-"));
  ...
  t.after(() => service.close());
  const space = { id: "space-parent", spaceRoot: root };
  const enabled = await service.enable({ space, proposalPath, actor: "human" });
  const accepted = await service.run({ space, checkId: enabled.declaration.id, actor: { kind: "cli" } });
  const run = await waitForTerminal(service, space.id, accepted.taskId);
...
test("Space removal purges local Check authority so re-registration cannot revive it", async (t) => {
  ...
  const options = {
    kernel: new WorkFoldKernel(),
    storeFactory: (spaceId: string) => WorkFoldCheckStore.create(spaceId, { path: statePath }),
    listSpaces: async () => [spaceSummary("space-removal", root)],
  };
  const service = new WorkFoldCheckService(options);
  t.after(() => service.close());
  const space = { id: "space-removal", spaceRoot: root };
