...
import type { WorkFoldCheckTarget } from "../src/shared/checks.js";
import {
  resolveWorkFoldCheckTargets,
  WorkFoldCheckTargetResolutionError,
  type WorkFoldCheckTargetResolutionErrorCode,
} from "../src/local/checks/target-resolver.js";

function file(path: string, role: "primary" | "reference" = "primary"): WorkFoldCheckTarget {
...
function resolutionError(code: WorkFoldCheckTargetResolutionErrorCode): (error: unknown) => boolean {
...
test("Check targets resolve exact files and explicitly filtered trees in deterministic order", async (t) => {
  ...
  const result = await resolveWorkFoldCheckTargets(root, [
  ...
  const again = await resolveWorkFoldCheckTargets(root, [
    file("Trip/z.txt", "reference"),
...
test("tree recursion and extension membership are always explicit", async (t) => {
  ...
  const shallow = await resolveWorkFoldCheckTargets(root, [tree("Data", [".csv"])]);
  assert.deepEqual(shallow.files.map((item) => item.path), ["Data/top.csv"]);
  const recursive = await resolveWorkFoldCheckTargets(root, [tree("Data", [".csv"], { recursive: true })]);
  assert.deepEqual(recursive.files.map((item) => item.path), ["Data/nested/child.csv", "Data/top.csv"]);

  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [tree("Data", [])]),
...
test("the resolver refuses the Space root, path escapes, and reserved Check material", async (t) => {
  ...
  for (const target of targets) {
    await assert.rejects(
      resolveWorkFoldCheckTargets(root, [target]),
      (error: unknown) => error instanceof WorkFoldCheckTargetResolutionError
...
test("reserved metadata nested below an explicit tree is never resolved", async (t) => {
  ...
  await writeFile(join(root, "Docs", ".work-fold", "current.md"), "hidden", "utf8");
  ...
  const result = await resolveWorkFoldCheckTargets(root, [tree("Docs", [".md"], { recursive: true })]);
...
test("exact missing files are data while missing trees and type mismatches are resolver errors", async (t) => {
  ...
  const missing = await resolveWorkFoldCheckTargets(root, [
  ...
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [tree("Missing", [".md"])]),
    resolutionError("TARGET_NOT_FOUND"),
  );
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [tree("Expected/not-a-directory.txt", [".txt"])]),
    resolutionError("TYPE_MISMATCH"),
  );
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [file("Expected")]),
    resolutionError("TYPE_MISMATCH"),
  );
});
...
test("exact and tree targets reject symbolic links and junctions", async (t) => {
  ...
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [file("linked.md")]),
    resolutionError("SYMLINK"),
  );
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [tree("linked-tree", [".md"], { recursive: true })]),
    resolutionError("SYMLINK"),
  );
  ...
  await assert.rejects(
    resolveWorkFoldCheckTargets(linkedRoot, [file("anything.md")]),
    resolutionError("SYMLINK"),
  );
});
...
test("file-count and byte bounds fail closed without returning a partial target set", async (t) => {
  ...
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [tree("Files", [".txt"])], { limits: { maxFiles: 2 } }),
    resolutionError("LIMIT_EXCEEDED"),
  );
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [file("Files/large.txt")], { limits: { maxFileBytes: 4 } }),
    resolutionError("LIMIT_EXCEEDED"),
  );
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [file("Files/a.txt"), file("Files/b.txt")], { limits: { maxTotalBytes: 5 } }),
    resolutionError("LIMIT_EXCEEDED"),
  );
  await assert.rejects(
    resolveWorkFoldCheckTargets(root, [tree("Files", [".no-match"])], { limits: { maxVisitedEntries: 2 } }),
    resolutionError("LIMIT_EXCEEDED"),
  );
});
