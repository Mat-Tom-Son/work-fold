...
test("CLI file broker atomically claims, executes, responds, and deduplicates in-flight work", async () => {
  await withBroker(async ({ broker, root }) => {
    ...
    const executor = async () => {
      calls += 1;
      await gate;
      return createWorkFoldCliResponse({ id, exitCode: 0, stdout: "done\n", stderr: "", result: { ok: true } });
    };
...
test("CLI broker paths are UUID-derived and cannot escape the injected state root", async () => {
  await withBroker(async ({ broker, root }) => {
    const id = randomUUID();
    const expected = workFoldCliBrokerPaths(root);
...
test("CLI broker turns stale, future, mismatched, malformed, and oversized requests into stable protocol responses", async () => {
  await withBroker(async ({ broker, root }) => {
    const cases: Array<{ id: string; value: unknown; expectedExit: number; expected: RegExp }> = [
      {
        ...
        expectedExit: WorkFoldCliExitCode.timeout,
        expected: /expired/,
      },
      {
        id: randomUUID(),
    ...
    const malformed = await broker.processRequest(malformedId, async () => createWorkFoldCliResponse({ id: malformedId, exitCode: 0, stdout: "", stderr: "" }));
    assert.equal(malformed.exitCode, WorkFoldCliExitCode.protocolError);
  ...
  await withBroker(async ({ broker }) => {
    ...
    const response = await broker.processRequest(id, async () => createWorkFoldCliResponse({ id, exitCode: 0, stdout: "", stderr: "" }));
    assert.equal(response.exitCode, WorkFoldCliExitCode.protocolError);
...
test("CLI broker claims, executes, and replays act-lane requests without re-executing mutations", async () => {
  await withBroker(async ({ broker, root }) => {
    ...
    let executions = 0;
    const executor = async (request: WorkFoldCliBrokeredRequest) => {
      executions += 1;
      assert.ok(isWorkFoldCliActRequest(request), "the executor must receive the act-lane request");
      assert.equal(request.actToken, "0".repeat(64));
      assert.equal(request.payload?.messageFile, "hello from a file");
      return createWorkFoldCliResponse({ id: request.id, exitCode: 0, stdout: "done\n", stderr: "", completedAt: fixedNow.toISOString() });
    };
...
test("CLI broker rejects symlinked request files", async (t) => {
  await withBroker(async ({ broker, sandbox, root }) => {
    ...
    await writeFile(external, JSON.stringify(createWorkFoldCliRequest({ id, argv: [], cwd: root, createdAt: fixedNow.toISOString() })), "utf8");
    try {
    ...
    const response = await broker.processRequest(id, async () => createWorkFoldCliResponse({ id, exitCode: 0, stdout: "", stderr: "" }));
    assert.equal(response.exitCode, WorkFoldCliExitCode.permissionDenied);
...
test("CLI broker rejects reparse-point directories", async (t) => {
  await withSandbox(async ({ sandbox, root }) => {
    const paths = workFoldCliBrokerPaths(root);
    ...
    const broker = new WorkFoldCliFileBroker({ stateRoot: root, now: () => fixedNow });
...
test("CLI response writer validates schema, bounds output, and refuses overwrite", async () => {
  await withBroker(async ({ broker }) => {
    const id = randomUUID();
    const response = createWorkFoldCliResponse({
      id,
      exitCode: 0,
      stdout: "ok\n",
      stderr: "",
      result: { protocolVersion: WORKFOLD_CLI_PROTOCOL_VERSION },
  ...
  await withBroker(async ({ broker }) => {
    const id = randomUUID();
    const response = createWorkFoldCliResponse({ id, exitCode: 0, stdout: "x".repeat(300), stderr: "" });
    await assert.rejects(() => broker.writeResponse(response), /size limit/);
  }, { maxResponseBytes: 256 });
});

async function withBroker(
  action: (context: { broker: WorkFoldCliFileBroker; sandbox: string; root: string }) => Promise<void>,
  limits: { maxRequestBytes?: number; maxActRequestBytes?: number; maxResponseBytes?: number } = {},
): Promise<void> {
  await withSandbox(async ({ sandbox, root }) => {
    const broker = new WorkFoldCliFileBroker({ stateRoot: root, now: () => fixedNow, ...limits });
