...
import type { WorkspaceCheckRunRecord } from "../src/local/checks/check-types.js";

const now = "2026-08-01T00:00:00.000Z";
const sensorDigest = workspaceFilePresenceSensorDigest;
...
test("Check authority is exact-digest machine state and disabling preserves its audit", async () => {
  const directory = await mkdtemp(join(tmpdir(), "workspace-check-store-"));
  const path = join(directory, "state.json");
  const store = await WorkspaceCheckStore.create("space-trip", { path });
  ...
  const reloaded = await WorkspaceCheckStore.create("space-trip", { path });
...
test("unfinished Check runs recover as interrupted and are never replayed", async () => {
  const directory = await mkdtemp(join(tmpdir(), "workspace-check-run-"));
  const path = join(directory, "state.json");
  const store = await WorkspaceCheckStore.create("space-trip", { path });
  const digest = workspaceCheckDigest(declaration);
  ...
  const recovered = await WorkspaceCheckStore.create("space-trip", { path });
...
test("future and damaged Check authority state fail closed", async () => {
  const directory = await mkdtemp(join(tmpdir(), "workspace-check-damaged-"));
  const path = join(directory, "state.json");
  await writeFile(path, JSON.stringify({ version: 99, revision: 0, authorizations: {}, decisions: {}, runs: [] }));
  await assert.rejects(() => WorkspaceCheckStore.create("space-trip", { path }), /unsupported version/);

  await writeFile(path, "{ definitely-not-json");
  await assert.rejects(() => WorkspaceCheckStore.create("space-trip", { path }), /could not read Check state/);
...
test("backup recovery preserves the last known good backup while repairing primary state", async () => {
  const directory = await mkdtemp(join(tmpdir(), "workspace-check-backup-"));
  const path = join(directory, "state.json");
  const store = await WorkspaceCheckStore.create("space-trip", { path });
  const digest = workspaceCheckDigest(declaration);
...
test("backup fallback never resurrects a disabled Check grant", async () => {
  const directory = await mkdtemp(join(tmpdir(), "workspace-check-revocation-backup-"));
  const path = join(directory, "state.json");
  const store = await WorkspaceCheckStore.create("space-trip", { path });
  const digest = workspaceCheckDigest(declaration);
  await store.authorize(declaration, digest, "human", sensorDigest, new Date(now));
...
test("repeating an identical decision is idempotent", async () => {
  const directory = await mkdtemp(join(tmpdir(), "workspace-check-decision-idempotent-"));
  const store = await WorkspaceCheckStore.create("space-trip", { path: join(directory, "state.json") });
  const input = {
