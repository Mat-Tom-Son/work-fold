...
export function CheckInbox() {
  ...
  return <section className="fold-section">
    <button type="button" className="fold-strip" aria-expanded={open} onClick={() => setOpen(!open)}>
      <span>{attention ? `Checks · ${attention} need attention` : "Checks"}</span>
    </button>
    {open ? <div className="fold-drawer">
      {error ? <p role="alert">Check status is unavailable.</p> : rows.map((row) => <div className="check-inbox-row" key={row.spaceId}>
        <div><strong>{row.spaceName}</strong><p>{row.needsAttention ? `${row.needsAttention} need attention` : row.errors ? "Review Check health" : row.neverRun ? "Ready for a first run" : row.state === "not-configured" ? "Proposal ready for review" : row.state === "current-clear" ? "Current · no findings" : "Review Checks"}</p></div>
